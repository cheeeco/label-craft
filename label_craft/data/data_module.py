...
class TextDataModule(pl.LightningDataModule):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.max_samples_per_category = cfg.data.data.get(
    ...
    def _limit_samples_per_category(self, texts, labels):
        ...
        for label in df["label"].unique():
            ...
            if category_count > self.max_samples_per_category:
                ...
                sampled_df = label_df.sample(
                    n=self.max_samples_per_category, random_state=42
                )
                category_stats[
                    label
                ] = f"{self.max_samples_per_category}/{category_count}"
            else:
                # Keep all samples
                sampled_df = label_df
        ...
        limited_categories = sum(
            1
            for v in category_stats.values()
            if "/" in v and v.split("/")[0] != v.split("/")[1]
        )
        total_categories = len(category_stats)

        logger.info(f"Category sampling limit: {self.max_samples_per_category}")
        logger.info(
            f"Categories with samples limited: "
            f"{limited_categories}/{total_categories}"
        )
